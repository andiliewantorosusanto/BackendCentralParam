...
public class RateAsuransiRestController {
    ...
    public Map<String, Object> getAllSkemaRateAsuransi(Principal principal, Pageable pageable,
            HttpServletResponse response) {
        Map<String, Object> crunchifyMap = new HashMap<String, Object>();
        String id = principal.getName();
        ...
        if (role.contains("USER")) {
            try {
                logger.info("Berhasil GET ALL Skema RateAsuransi");
                crunchifyMap.put("dataSkema", rateAsuransiRepository.getListDataSkemaRateAsuransi());
                crunchifyMap.put("code", "1");
            } catch (Exception e) {
                logger.error("ERROR");
                response.setStatus(400);
